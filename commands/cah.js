module.exports = {
  name: 'cah',
  aliases: ['cards-against-humanity'],
  description: 'cards against humanity',

  execute: () => {
    let black = blackCards[Math.floor(Math.random() * blackCards.length)];
    let whiteList = [];

    let count = (black.match(/___/g) || []).length;
    let trueCount = count;
    count === 0 ? count = 1 : count = count;

    for(let i = 0; i < count; i++) {
      let w = (whiteCards[Math.floor(Math.random() * whiteCards.length)]);
      whiteList.push(w.slice(0, -1));
    }

    if(trueCount === 0) { return `${ blackCard } **${ whiteList[0] }**`; }
    
    for(let i = 0; i < trueCount; i++) {
      blackCard = blackCard.replace('___', `__${ whiteCardList[i] }__`);
    }
    return blackCard;
